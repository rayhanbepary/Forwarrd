...
class AllCollectorsTable extends Component {
    ...
    loadMoreData() {
        const allCollectors = this.state.orgTableData;
        
        const slice = allCollectors.slice(this.state.offset, this.state.offset + this.state.perPage)
        this.setState({
            pageCount: Math.ceil(allCollectors.length / this.state.perPage),
            tableData:slice
        })
    ...
    getAllCollectors () {
        ...
        .then(response => {
            let allCollectors = response.data;
            let slice = allCollectors.slice(this.state.offset, this.state.offset + this.state.perPage);

            this.setState({
                orgTableData: allCollectors,
                tableData: slice,
                pageCount: Math.ceil(allCollectors.length / this.state.perPage)
            })
        })
    ...
    render() {
        ...
        return (
            <div className="admin-main">
                <button className="add-deposit" onClick={this.openCreateModal}>Add new collector</button>
                <CreateCollector
                                     ...
                                     <table width="100%" className="collections-table">
                                        ...
                                        <tbody>
                                            { filteredData.length > 0 ?
                                                    filteredData.map(result => 
                                                        <tr key={result._id}>
                                                            ...
                                                            <td data-label="Actions" style={{fontSize: '20px',color: '#008000'}}>
                                                                <Link to={`/collector-details/${result._id}`} style={{color: '#008000'}} title="View details"><FiExternalLink/></Link>
                                                ...
                                                : <tr><td>No Collector Found</td></tr>
                                            }
                                        </tbody>
                                    </table>
                                </div>
...
export default connect(null)(AllCollectorsTable);